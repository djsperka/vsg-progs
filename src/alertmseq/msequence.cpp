...
#if 0	
	// Now draw a 16x16 grid in video memory
	// Note the use of (0.5) in the calculation of the x and y coords. 
	// The entire grid is drawn starting in the upper left hand corner of video memory. 
	// The 0,0 term is drawn at (2.0, 0.5), which apparently means that the rect covers pixels 1-4 in x, and pixel 1 in y. 
	// The offset for this position is (-W/2 + w/2) -- this is subtracted from the origin of the video page prior to display.
	// In our case, this would ADD half the screen width, and subtract half the width of the grid. 
	vsgSetPen2(0);
	vsgSetCommand(vsgVIDEOCLEAR);
...
#if 0
	// mposition for first term
	MPositions[0].Page = 0+vsgDUALPAGE+vsgTRIGGERPAGE ;
	MPositions[0].Xpos=-W/2 + w/2 - f_apX;				// + column*pixWidth;
	MPositions[0].Ypos=-H/(2*f_iZoom) + h/(2*f_iZoom) + f_apY/f_iZoom;				// + row*pixHeight;		pixHeight takes ZOOM into account
	MPositions[0].Frames=f_iFramesPerTerm;
	MPositions[0].Stop=0;
	MPositions[0].ovPage=0;
	MPositions[0].ovXpos=0;
	MPositions[0].ovYpos=0;

	vsgPageCyclingSetup(1,&MPositions[0]);
	vsgSetCommand(vsgCYCLEPAGEENABLE);
...
{	
	...
	extern int optind;
	int errflg = 0;
	while ((c = getopt(argc, argv, "f:o:r:c:R:vp:d:")) != -1)
	{
		switch (c) 
		{
		case 'p':
		...
		case 'v':
			...
			break;
		case 'R':
			f_pr = new RepeatingSpec(vsgGetSystemAttribute(vsgFRAMETIME));
			s.assign(optarg);
			if (parse_repeating(s, *f_pr))
			{
				cerr << "Cannot parse repeating spec (" << s << ")." << endl;
				errflg++;
			}
			else
			{
				have_mode = true;
				f_isRepeating = true;
				cout << "Repeating spec: " << f_pr->getRepeats() << " x frames " << f_pr->getFirst() << "-" << f_pr->getLast() << endl;
			}
			break;
		case 'h':
			errflg++;
