...
/////////////////////////////////////////////////////////////////////////////////////
int DrawGrating(double SpatialFrequency, double TemporalFrequency, double Contrast,
				double Orientation)
{
...
//Draw the grating centered in the middle of the screen.
  ...
  vsgObjSetDriftVelocity(TemporalFrequency);
...
int SetupOverlay ( double GratingDiameter, double DotDiameter , double DotSurround,double DotContrast)
{
	...
	for (index=0; index < 4; index++)
		vsgSetDrawPage(vsgOVERLAYPAGE, index, 1);
...
UINT main(int ArgumentCount, char *Arguments[])
{
	...
	double GratingDiameter, GratingContrast, SpatialFrequency, TemporalFrequency, Orientation;
	double DotDiameter, DotContrast, DotSurround;
	...
	TemporalFrequency=atof(Arguments[4]);
	Orientation=atof(Arguments[5]);
	DotDiameter=atof(Arguments[6]);
	DotContrast=atof(Arguments[7]);
	DotSurround=atof(Arguments[8]);
	DistanceToScreen=atof(Arguments[9]);
	FramesPerTerm=atoi(Arguments[10]);
	X=atof(Arguments[11]);
	Y=atof(Arguments[12]);
	Repeats=atoi(Arguments[13]);
	RunTime=atof(Arguments[14]);
	BlankTime=atof(Arguments[15]);
	...
	DrawGrating(SpatialFrequency,TemporalFrequency,GratingContrast, Orientation);
	SetupOverlay(GratingDiameter, DotDiameter, DotSurround, DotContrast);
	HANDLE hThread;
    unsigned threadID ;
	Sleep(2000);
//	vsgSetZoneDisplayPage(vsgVIDEOPAGE,0+vsgTRIGGERPAGE);
