...
void main(int ArgumentCount, char *Arguments[])
{
	...
	double RandBase,RandChange;
	CString BackgroundColor,GratingOrSpot,GratingColor;
	double TargetOrientation, TargetSF, TargetTF, TargetDiameter;
	int index,StimOver, ConstantDiff;
	int NumberOfDistractors,PixelRange, temp2,temp3,ConIndex;
	int GratingConfig=0;
	double NumberBaseContrasts,NumberPerChanges,LowBaseContrast, HighBaseContrast,StartCondition;
	double temp,LowPerChange,HighPerChange;
	float BaseContrast[20],PerChange[20];
	...
	ConstantDiff=atof(Arguments[16]);
	StartCondition=atof(Arguments[17]);
	printf("Const Diff = %i\n",ConstantDiff);
	FILE *ConstDiffFile;
	if (ConstantDiff==0)
	{
	...
	{
		NumberBaseContrasts=6;
		NumberPerChanges=6;
		ConstDiffFile = fopen("d:\\AlertRig-050715\\AttentionData\\ConstDiffInfo.txt","r");
		printf("Here Scan\n");
		if(ConstDiffFile == NULL)
			printf( "The file ConstDiffInfo.txt.out was not opened\n" );
   
		for (index=0; index<6; index++)
		{
			fscanf(ConstDiffFile,"%f%f",&BaseContrast[index],&PerChange[index]);
			
			printf("BaseContrast = %f\n",BaseContrast[index]);
	...
	int CurrentComCount, index2, index3;
	...
	if (ConstantDiff==0)
	{
		...
		{
			RandBase=floor((NumberBaseContrasts*rand())/RAND_MAX);
			index2=RandBase;
			RandChange=floor((NumberPerChanges*rand())/RAND_MAX);
			index3=RandChange;
		//	if (RandBase>5)
		//		RandBase = 5;
		//	if (RandChange>5)
		...
		//	printf("Base = %d\n", PerChange[index3]);
			...
			RandIntContrast[index]=BaseContrast[index2];
			ContrastUp[index]=BaseContrast[index2]+(BaseContrast[index2]*PerChange[index3]);
			ContrastDown[index]=BaseContrast[index2]-(BaseContrast[index2]*PerChange[index3]);
			DisRandIntContrast[index]=BaseContrast[index2];
			DisContrastUp[index]=BaseContrast[index2]+(BaseContrast[index2]*PerChange[index3]);
			DisContrastDown[index]=BaseContrast[index2]-(BaseContrast[index2]*PerChange[index3]);
	...
	else
	{
		for (index=0; index<5000;index++)
		{
			RandBase=floor((6*rand())/RAND_MAX);
			index2=RandBase;
			RandChange=RandBase;//floor((6*rand())/RAND_MAX);
			index3=RandChange;
		//	if (RandBase>5)
		//		RandBase = 5;
		//	if (RandChange>5)
	...
	vsgSetViewDistMM(550); //MAKE THIS VARIABLE!!
	...
	else
	{
		...
		{
			vsgDrawGrating(Distractors[0][5],Distractors[0][6],TargetDiameter,TargetDiameter,
							TargetOrientation-90,TargetSF);
		}
	}
		...
		else
		{
				...
				
			{
				...
				vsgDrawGrating(Distractors[0][5],Distractors[0][6],Distractors[0][3],Distractors[0][3],
							TargetOrientation-90,Distractors[0][1]);
	...
	while (TRUE)
	{
		...
		if(NonJuicePins != PreviousState)
		{
			...
			fprintf(ProgActFile,"Answer Point Value %i \n",AnswerPoint);
			fprintf(ProgActFile,"Answer Fixation Point Value %i \n",PreviousAnswerPoint);
            fclose(ProgActFile);
			ProgActFile = fopen(CurrentExperiment3,"a");
		//	printf("Advance trial here = %i/n",Fixation);
			...
			if (AdvanceTrial != PreviousAdvanceTrial)
			{	
				printf("Advance Trial!!!!!!!!!!!!!!!!!!!!!!!!!\n");
			...
			{	
				...
				case 1:
					{
						fprintf(ProgActFile,"Contrast Up Original Target Trial, Contrast = %f\n",ContrastUp[ConIndex]);
				...
				case 0:
					{
						fprintf(ProgActFile,"Contrast Down Original Target Trial, Contrast = %f\n",ContrastDown[ConIndex]);
						printf("Initial Contrast = %f\n",RandIntContrast[ConIndex]);
						printf("Conindex = %i\n",ConIndex);
			...
			{	
				...
				case 1:
					{
						fprintf(ProgActFile,"Contrast Up Original Distractor Trial, Contrast = %f\n",DisContrastUp[ConIndex]);
				...
				case 0:
					{
						fprintf(ProgActFile,"Contrast Down Original Distractor Trial Contrast = %f\n",DisContrastDown[ConIndex]);
						printf("Initial Contrast =%f\n",DisRandIntContrast[ConIndex]);
						printf("Conindex = %i\n",ConIndex);
