/* $Id: msequence.cpp,v 1.2 2011-06-23 02:09:34 djsperka Exp $ */
...
int main(int argc, char **argv)
{
	...
	for (int iterm = f_pr->getFirst(); iterm <= f_pr->getLast(); iterm++)
	{
		...
		int irow = (index%128) * f_iRows + (int)floor((float)index/(float)(128*16));
		int icol = (int)(floor((float)index/128.0f))%f_iCols;
...
{	
	...
	bool have_d=false;
	bool have_N=false;		// this will check args and exit!
	...
	extern int optind;
	int errflg = 0;
	while ((c = getopt(argc, argv, "f:R:vp:d:N")) != -1)
	{
		switch (c) 
		{
		case 'N':
		...
		case 'R':
			...
			else
			{
				have_mode = true;
				f_isRepeating = true;
				cout << "Repeating spec: " << f_pr->getRepeats() << " x frames " << f_pr->getFirst() << "-" << f_pr->getLast() << " (frametime=" << vsgGetSystemAttribute(vsgFRAMETIME) << " Total time=" << f_pr->getTimeUS() <<  " us)" << endl;
			}
			break;
		case 'h':
			errflg++;
