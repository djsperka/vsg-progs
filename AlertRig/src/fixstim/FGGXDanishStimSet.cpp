...
std::string FGGXDanishStimSet::toString() const
{
	std::ostringstream oss;
	oss << "FGGXDanishStimSet NI.";
	return oss.str();
}

void FGGXDanishStimSet::initialize(double xOffset, double yOffset)
{
	double x, y;
...
void FGGXDanishStimSet::per_trial_predraw_updates()
{
	double ori;
	double diam;
...
int FGGXDanishStimSet::init(ARvsg& vsg, std::vector<int> pages)
{	
	int status = 0;
	vsg.select();
...
void FGGXDanishStimSet::draw_pages(bool bDrawAllPages)
{
	int savepage;
	double x, y;
	int lr;
