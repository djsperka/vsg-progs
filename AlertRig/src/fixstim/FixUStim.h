...
// passing a command file.
	...
	bool parseImageArg(const std::string& arg, std::string& filename, double& x, double& y, double& duration, int& nlevels, int& low_water, int& hi_water);
	bool parsePlaidArg(const std::string& arg, double& plX, double &plY, double& plX0, double& plY0, std::vector<double>& vecContrast, std::vector<double>& vecTF, std::vector<double>& vecOri);
