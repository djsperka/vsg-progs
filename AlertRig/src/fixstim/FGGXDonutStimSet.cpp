...
std::string FGGXDonutStimSet::toString() const
{
	std::ostringstream oss;
	oss << "FGGXDonutStimSet NI.";
	return oss.str();
}

void FGGXDonutStimSet::per_trial_predraw_updates()
{
	double diam;
...
void FGGXDonutStimSet::draw_pages(bool bDrawAllPages)
{
	int savepage;
	double x, y;
	int lr;
...
int FGGXDonutStimSet::init(ARvsg& vsg, std::vector<int> pages)
{	
	int status = 0;
	vsg.select();
