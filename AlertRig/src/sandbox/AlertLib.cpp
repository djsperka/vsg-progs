...
int alert::ARvsg::init_overlay()
{
	...
	{		
		...
		{
			int numOverlay = vsgGetSystemAttribute(vsgNUMOVERLAYPAGES);
			int i;
			cout << " There are " << numOverlay << " overlay pages." << endl;
			vsgPaletteWriteOverlayCols((VSGLUTBUFFER*)&background, 1, 1);

			for (i=numOverlay-1; i>=0; i--) vsgSetDrawPage(vsgOVERLAYPAGE, i, 1);
		}
	}
	return status;
}
