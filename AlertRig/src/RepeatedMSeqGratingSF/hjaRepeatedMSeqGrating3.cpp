...
{
	int CheckCard, FPT, NumTerms,X,Y, i, DistanceToScreen;
	double Height, Width,SFd, Ori, Con, SmallSize, LargeSize;
	int DrawGrating(int FPT, double SFd, double Ori, double Con, double SmallSize, double LargeSize, double X, double Y, int NumTerms, int DistanceToScreen);

	VSGTRIVAL Background;
	
	Background.a = 0.5;	Background.b = 0.5;	Background.c = 0.5;

	FPT=atoi(Arguments[1]);
	SFd = atof(Arguments[2]);
	Ori = atof(Arguments[3]);
	Con = atof(Arguments[4]);
	SmallSize = atof(Arguments[6]);
	LargeSize = atof(Arguments[7]);
	X = atoi(Arguments[8]);
	Y = atoi(Arguments[9]);
	NumTerms = atoi(Arguments[10]);
	DistanceToScreen = atoi(Arguments[11]);
	i=0;
	CheckCard = vsgInit("");
	if (CheckCard < 0)
	{
	...
	DrawGrating(FPT,SFd,Ori,Con,SmallSize,LargeSize,X,Y,NumTerms,DistanceToScreen);
}


int DrawGrating(int FPT, double SFd, double Ori, double Con, double SmallSize, double LargeSize, double X, double Y, int NumTerms, int DistancetoMonitor)
{
	int i, j,k, x, y,z, dx,dy, OD, OD2,index2, index4;
	...
	for (i= 0; i<PIXELSpCYCLE; i++)
	{
		...
		TempBuffer2[1].Buffer[i].a =(sin(2*Pi*(index/PIXELSpCYCLE)+180)/2)+.5;
		TempBuffer2[1].Buffer[i].b =(sin(2*Pi*(index/PIXELSpCYCLE)+180)/2)+.5;
		TempBuffer2[1].Buffer[i].c =(sin(2*Pi*(index/PIXELSpCYCLE)+180)/2)+.5;
		GrayBuffer[i].a =.5;
		GrayBuffer[i].b =.5;
		GrayBuffer[i].c =.5;	
	}	
	...
	vsgLUTBUFFERWrite(65, (VSGLUTBUFFER*)&GrayBuffer);
	index=0;
	x=0;
	OD = 580;
	OD2 = OD/2;
