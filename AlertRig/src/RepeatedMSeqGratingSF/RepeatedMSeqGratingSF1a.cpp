...
int DrawGrating(int FPT, double SFd[2], double Ori, double Con, double Size, double X, double Y, double Signal, double Blank, int DistancetoMonitor,int Repeats)
{
	...
	PIXELSpCYCLE[1] = floor(MMpCYCLE[1]/ MMpPIXEL);
	...
	for (i= 1; i<=PIXELSpCYCLE[0]; i++)
	{
		...
		TempBuffer2[0].Buffer[i].a =(sin(2*Pi*(index/PIXELSpCYCLE[0])+0)/2)+.5;
		TempBuffer2[0].Buffer[i].b =(sin(2*Pi*(index/PIXELSpCYCLE[0])+0)/2)+.5;
		TempBuffer2[0].Buffer[i].c =(sin(2*Pi*(index/PIXELSpCYCLE[0])+0)/2)+.5;
		TempBuffer2[1].Buffer[i].a =(sin(2*Pi*(index/PIXELSpCYCLE[0])+Pi)/2)+.5;
		TempBuffer2[1].Buffer[i].b =(sin(2*Pi*(index/PIXELSpCYCLE[0])+Pi)/2)+.5;
		TempBuffer2[1].Buffer[i].c =(sin(2*Pi*(index/PIXELSpCYCLE[0])+Pi)/2)+.5;
	...
	SinFile = fopen("C:\SinFile.txt","w");
		for (i= 1; i<=PIXELSpCYCLE[1]; i++)
	{
		fprintf(SinFile,"%f\n",(sin(2*Pi*(index/PIXELSpCYCLE[1])+0)/2)+.5);
		TempBuffer2[2].Buffer[i].a =(sin(2*Pi*(index/PIXELSpCYCLE[1])+0)/2)+.5;
		TempBuffer2[2].Buffer[i].b =(sin(2*Pi*(index/PIXELSpCYCLE[1])+0)/2)+.5;
		TempBuffer2[2].Buffer[i].c =(sin(2*Pi*(index/PIXELSpCYCLE[1])+0)/2)+.5;
		TempBuffer2[3].Buffer[i].a =(sin(2*Pi*(index/PIXELSpCYCLE[1])+Pi)/2)+.5;
		TempBuffer2[3].Buffer[i].b =(sin(2*Pi*(index/PIXELSpCYCLE[1])+Pi)/2)+.5;
		TempBuffer2[3].Buffer[i].c =(sin(2*Pi*(index/PIXELSpCYCLE[1])+Pi)/2)+.5;
		TempBuffer2[4].Buffer[i].a =.5;
		TempBuffer2[4].Buffer[i].b =.5;
		TempBuffer2[4].Buffer[i].c =.5;	
		index=index+1;
	...
	double NumCycles[2];
	int z2;
	NumCycles[0] = (OD/PIXELSpCYCLE[0]);
	NumCycles[1] = (OD/PIXELSpCYCLE[1]);
	...
	for (z = 0; z<=1; z++)
	{
		...
		MaxVal = (cos(Ori)*(900)) + (sin(Ori)*(900));
		printf("PIXELSpCYCLE %i\t  = %f\n",z2,PIXELSpCYCLE[z2]);
		vsgSetDrawPage(vsgVIDEOPAGE,z,vsgBACKGROUND);
		vsgSetPen1(0);
		vsgDrawRect(0,0,900,900);
		
		for (x= -450; x<=450; x=x+1)
		{
		
			for (y= -450; y<=450; y=y+1)
			{			
				xcor = (cos(Ori)*(x+(450)));
				ycor = sin(Ori)*(y+450);
